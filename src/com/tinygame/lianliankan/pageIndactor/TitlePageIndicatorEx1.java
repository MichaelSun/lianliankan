 ...
 * also scrolled.
 */
public class TitlePageIndicatorEx1 extends View implements PageIndicator {
    /**
    ...
    public TitlePageIndicatorEx1(Context context) {
        this(context, null);
    }

    public TitlePageIndicatorEx1(Context context, AttributeSet attrs) {
...
//    }
    ...
    private ArrayList<RectF> calculateAllBounds(Paint paint) {
        ...
        final int width = getWidth();
        final int halfWidth = width / 2;
        final int titleOffset = (int) ((mCurrentOffset - (mSelectedPage-mCurrentPage)*width)/2f);
        ...
        //middle
        {
	        RectF bounds = calcBounds(mSelectedPage, paint);
	        float w = (bounds.right - bounds.left);
	        float h = (bounds.bottom - bounds.top);
	        bounds.left = (halfWidth) - (w / 2) - titleOffset;
	        bounds.right = bounds.left + w;
	        bounds.top = 0;
	        bounds.bottom = h;
	        list.remove(mSelectedPage);
	        list.add(mSelectedPage, bounds);
        }
        //left one
        {
        	if(mSelectedPage > 0){
	            RectF bounds = calcBounds(mSelectedPage-1, paint);
	            float w = (bounds.right - bounds.left);
	            float h = (bounds.bottom - bounds.top);
	            bounds.left = 0 - titleOffset;
	            bounds.right = bounds.left + w;
	            bounds.top = 0;
	            bounds.bottom = h;
	            list.remove(mSelectedPage-1);
		        list.add(mSelectedPage-1, bounds);
        	}
        }
        //right one
        {
	        if(mSelectedPage < count-1){
	            RectF bounds = calcBounds(mSelectedPage+1, paint);
	            float w = (bounds.right - bounds.left);
	            float h = (bounds.bottom - bounds.top);
	            bounds.right = getLeft()+getWidth() - titleOffset;
